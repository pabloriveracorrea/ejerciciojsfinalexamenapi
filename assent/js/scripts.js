...
async function renderizarGrafico() {
  ...
  const datos = await fetch(
    `https://mindicador.cl/api/${valorSeleccionadoConvertir.toLowerCase()}`
  );
  const datas = await datos.json();
  /* realizando data para graficacion */
  const labels = datas.serie.slice(0, 10).map((item) => {
    return item.fecha.substring(0, 10);
  });
  // console.log('labels', labels);
  const valores = datas.serie.slice(0, 10).map((item) => {
    return item.valor;
  });
  //console.log(valores);
  const config = {
